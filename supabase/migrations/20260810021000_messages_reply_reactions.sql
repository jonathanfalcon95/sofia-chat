-- Reply quotes + reactions on WhatsApp messages (YCloud).
...
create index if not exists messages_reply_to_wamid_idx
  on public.messages (reply_to_wamid)
  where reply_to_wamid is not null;

-- Backfill: inbound ids are usually already wamids.
update public.messages
set wamid = ycloud_message_id
where wamid is null
  and ycloud_message_id is not null
  and ycloud_message_id like 'wamid.%';
...
begin
  ...
  if p_event_type in ('whatsapp.inbound_message.received', 'whatsapp.inbound.message') then
    ...
    -- Reactions update the target message; do not create a bubble or bump unread.
  ...
  if p_event_type = 'whatsapp.message.updated' then
    ...
    insert into public.messages (
      conversation_id, company_id, direction, type, body, ycloud_message_id, wamid, status, raw_payload
    ) values (
      v_conversation_id, v_inbox.company_id, v_direction, v_msg_type, v_body, v_msg_id, v_wamid, v_status, v_msg
    )
    on conflict (ycloud_message_id) do update set
      status = excluded.status,
      raw_payload = excluded.raw_payload,

-- Media messages often have WhatsApp wamid only inside raw_payload while the
-- messages.wamid column stayed null (YCloud id != wamid). Replies/reactions
-- need the column populated.

update public.messages
set wamid = coalesce(
  nullif(raw_payload->>'wamid', ''),
  nullif(raw_payload->'send'->>'wamid', ''),
  nullif(raw_payload->'whatsappMessage'->>'wamid', ''),
  case
    when ycloud_message_id like 'wamid.%' then ycloud_message_id
    else null
  end
)
where wamid is null
  and (
    nullif(raw_payload->>'wamid', '') is not null
    or nullif(raw_payload->'send'->>'wamid', '') is not null
    or nullif(raw_payload->'whatsappMessage'->>'wamid', '') is not null
    or ycloud_message_id like 'wamid.%'
  );

-- Keep wamid in sync on status updates (covers outbound media after send).
...
begin
  ...
  if p_event_type = 'whatsapp.message.updated' then
    ...
    set
      status = v_status,
      raw_payload = case
        -- Preserve local media storage metadata when status webhooks overwrite payload.

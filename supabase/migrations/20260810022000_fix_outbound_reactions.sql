-- Outbound reactions arrive via whatsapp.message.updated and were incorrectly
-- inserted as chat bubbles ("[reaction]"). Apply them to the target message instead.
...
begin
  ...
  if p_event_type = 'whatsapp.message.updated' then
    ...
    -- Never materialize reactions as chat bubbles.
    if v_msg_type = 'reaction' then
      v_reaction_target := coalesce(
        nullif(v_msg->'reaction'->>'message_id', ''),
        nullif(v_msg->'reaction'->>'wamid', ''),
...
-- Remove orphan reaction bubbles already persisted.
delete from public.messages where type = 'reaction';

-- Repair conversation previews polluted by reaction bubbles.
...
from (
  ...
  where m.type is distinct from 'reaction'
  order by m.conversation_id, m.created_at desc
) sub
where c.id = sub.conversation_id
  and c.last_message_preview = '[reaction]';
